...
# main df to be filtered ----
 ...
 filtered_player_df <- reactive({
   ...
   player_df %>%
      ...
      filter(cost_abbrv >= !!input$cost_range[1] & cost_abbrv <= !!input$cost_range[2]) %>% 
...
# Most Valuable Player panel ----
 mvp_profile <- reactive({
   ...
   #
   difficulty_df = data.frame(rating = 1:5,
                              colour = as.character(c("#02884e",
                                                    "rgb(1, 252, 122)",
                                                    "rgb(231, 231, 231)",
                                                    "rgb(255, 23, 81)",
                                                    "rgb(128, 7, 45)")),
   ...
   if(is.list(top_val_player_details$fixtures)){
     
     return(div(HTML("<i>Could not fetch mvp details. 
   ...
   colour_html <- paste0("<p>", paste('<span style=\"color:',
                                      colour_difs,
                                      '\">&#9632;</span>', collapse = ""), "</p>")
  #
   ...
   return(
    tagList(
     ...
     wellPanel(
       ...
       div(HTML("<i>Could not fetch player image. 
...
shinyWidgets::pickerInput(inputId = "position_select",
              label = "Select player position",
              choices = c("Goalkeeper", "Defender", "Midfielder",  "Forward"),
              selected = "Forward",
              multiple = TRUE)

  shinyWidgets::pickerInput(inputId = "team_select",
                            label = "Select Team(s)",
                            choices = unique(player_df$team_name),
                            selected = unique(player_df$team_name),
                            options =
                              list(`actions-box` = TRUE,                       `selected-text-format` = paste0("count > ", length(unique(player_df$team_name)) -1),                                          `count-selected-text` = "All"),
                            multiple = TRUE)



  sliderInput("cost_range",
              label = "Select Cost Range",
              min = 3.5,
              max = max(player_df$cost_abbrv, na.rm = TRUE),
              value = c(min(player_df$cost_abbrv, na.rm = TRUE),max(player_df$cost_abbrv, na.rm = TRUE)),
              post = "m",
              step = 0.5
              )
